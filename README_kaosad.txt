HOWTO (under Windows MingW32 -- kaosad 28/04/15)
================================================
To build, you have to run 'autogen.sh' script provided in this directory.
The script will run 'autoconf' and other scripts in MingW's 'bin' folder
(so make sure the PATH is properly set, e.g., 'PATH=$PATH:/c/mingw/bin'
if you need to).

But before you do that, you need to mount the 'mingw' directory first.
Otherwise you will receive an error. For example, if your MingW is
installed in C:\mingw, then type:

  mount C:\mingw \mingw

[see: https://github.com/irungentoo/toxcore/issues/719]

Now you are ready to run './autogen.sh', './configure', and finally
'make'.

[Note:
1) If you accidentally mounted wrongly, you need to un-mount first
   before re-'mount'ing again by issuing the following command:
     umount \mingw

2) I have no success with Git installed together with Mingw (the error
   did not go away), but no problem encountered on machine with no
   Git installed. The reason I believe is that Git has its own program/
   scripts that mess up with Msys of Mingw.
]
